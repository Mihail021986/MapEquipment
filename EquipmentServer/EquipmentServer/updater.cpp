...
void Updater::run(){
    ...
    if(db.open()){
        ...
        if(!upBlock.exec(query)){
            ...
            std::wcout<<QString("Данные по Block ").toStdWString().c_str()<<this->block.name.toStdWString().c_str()<<" c IP: "<<this->block.IP.toStdWString().c_str()<<" обновлены."<<std::endl;
        existID.clear();
        ...
        if(!upBlock.exec(query)){
            ...
            std::wcout<<QString("Данные по Board ").toStdWString().c_str()<<this->block.boards[i].name.toStdWString().c_str()<<" c N: "<<this->block.boards[i].num<<" обновлены."<<std::endl;
        for(int j=0; j<this->block.boards[i].ports.count(); j++){
            ...
            if(!upPort.exec(query)){
                ...
                std::wcout<<QString("Данные по Port c №").toStdWString().c_str()<<this->block.boards[i].ports[j].num<<" обновлены."<<std::endl;
        }
    }
    existID.clear();
    }
